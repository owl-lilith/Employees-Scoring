...
class YOLO:
    def __init__(self, obj_threshold, nms_threshold):
        ...
        data_folder_path = os.path.join(current_directory, '..', 'Resources')
        yolo_model_path = os.path.join(data_folder_path, 'yolo.h5')
        self._yolo = load_model(yolo_model_path)

    def _sigmoid(self, x):
        return 1 / (1 + np.exp(-x))

    def process_features(self, out, anchors, mask):
        """process output features.

        # Arguments
            out: Tensor (N, N, 3, 4 + 1 + 1), output feature map of yolo.
        ...
        # Returns
            ...
            box_class_probs: ndarray (N, N, 3, 1), class probs for per box.
        """
        ...
        box_class_probs = np.expand_dims(box_class_probs, axis=-1)
    ...
    def _yolo_out(self, outs, shape):
        ...
        for out, mask in zip(outs, masks):
            ...
            output_only_person = np.zeros((out.shape[0],out.shape[1],out.shape[2],out.shape[3],6))
            output_only_person[..., :6] = out[..., :6]


            b, c, s = self.process_features(output_only_person, anchors, mask)
    ...
    def predict(self, image, shape):
        ...
        outs = self._yolo.predict(image, verbose=0)
        boxes, classes, _ = self._yolo_out(outs, shape)
        ids = []
        if boxes is not None:
            ids = [str(box[0])+ str(box[1])+ str(box[2])+ str(box[3]) for box in boxes]
        return boxes, classes, ids

def main():
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..data/dataset/0.png')
